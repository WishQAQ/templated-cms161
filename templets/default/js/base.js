...
(function ($) {
    ...
    $.fn.sidebar = function (option) {
        var defaults = {
            subWrapClass: "sidebar-navSub",
            hoverClass: "navSubHover"
        };
        var isLock = null;

        var opts = $.extend(defaults, option);
        var $lis = this.children("li").not(".oncurr");
        $lis.each(function () {

            $(this).hover(function () {
                var $this = $(this);
                if ($(this).find(".sidebar-navSub").find("a").length <= 0) {
                    return false
                }
                ...
                if (isLock) {
                    ...
                    $this.children("." + opts.subWrapClass).stop(true, true).slideDown();
                }

            }, function () {
                $(this).children("." + opts.subWrapClass).stop(true, true).slideUp(100, function () {
                    isLock = false;
                });
            })
